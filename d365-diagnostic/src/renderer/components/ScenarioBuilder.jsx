...
const TX_TYPES = {
  lease: [
    { value: 'recognition',     label: 'Lease Initial Recognition' },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748b' }}>
            Define expected vs actual journal entries. The engine will compare them and identify root causes.
          </p>
        </div>
...
function EmptyState({ onAdd }) {
  return (
    <div style={S.empty}>
      ...
      <p style={{ fontSize: 13, color: '#475569', marginBottom: 16, maxWidth: 340, textAlign: 'center' }}>
        Add a scenario to compare expected accounting entries against what was actually posted in D365.
      </p>
...
const S = {
  ...
  th: { padding: '6px 6px 6px 0', textAlign: 'left', fontSize: 10, fontWeight: 600,
        textTransform: 'uppercase', letterSpacing: .5, borderBottom: '1px solid #1e293b' },
  td: { padding: '4px 4px 0 0', verticalAlign: 'top' },
  inp: {
    width: '100%', background: '#1a1f2e', border: '1px solid #1e293b', borderRadius: 4,
    color: '#e2e8f0', padding: '5px 7px', fontSize: 12, outline: 'none',
  },
  delRow: { background: 'none', border: 'none', color: '#475569', cursor: 'pointer', padding: '5px 4px', fontSize: 13 },
  addRowBtn: {
    marginTop: 8, background: 'none', border: '1px dashed', borderRadius: 4,
    padding: '4px 12px', fontSize: 12, cursor: 'pointer', width: '100%',
  },
  addBtn: {
  ...
  empty: {
    ...
    padding: '60px 40px', border: '2px dashed #1e293b', borderRadius: 12,
  },
};
